...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    sections: {
      ...
      scout: scoutRowsForSection.map((s: any) => ({ id: s.id, user: fullName(profileMap.get(s.user_id)) || s.metadata?.access_status || 'Guest', email: profileMap.get(s.user_id)?.email ?? null, question: short(s.question ?? s.content, 160), created_at: s.created_at })),
